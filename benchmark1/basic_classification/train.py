...
class ClassificationTrainer(object):
    ...
    def run(self, data_loader, model, optimizer, stage,
        ...
        if mode == ModeType.EVAL:
            ...
            ## TODO change the main evaluate metric
            # return fscore_list[0][cEvaluator.MICRO_AVERAGE]
            return fscore_list[0][cEvaluator.MACRO_AVERAGE]
...
def train(conf):
    ...
    for epoch in range(conf.train.start_epoch,
        ...
        if patience_epoch > patience:
            logger.info("Ran out of patience after {} epochs, and best f1 is {}".format(epoch, best_performance))
            break
    # best model on validateion set
    # best_epoch_file_name = model_file_prefix + "_" + str(best_epoch)
    # best_epoch_file_name = model_file_prefix
    ...
    #                 optimizer)
    load_checkpoint(model_file_prefix, conf, model,optimizer)
    trainer.eval(test_data_loader, model, optimizer, "Best test", best_epoch)
    logger.info("learning rate: {}".format(conf.optimizer.learning_rate))
